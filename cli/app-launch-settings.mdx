...
|--------|----------|----------|
| [CLI Flag](#cli-flag) | Quick one-off testing | Highest |
...
<Warning>
  The `--app-launch-options` flag uses option parsing that stops at values starting with `-`. For arguments like `-AppleLanguages` or `-SkipOnboarding`, use the config file or local settings instead.
</Warning>
...
### Arguments That Need Config File
...
Use the config file or local settings for these instead.
