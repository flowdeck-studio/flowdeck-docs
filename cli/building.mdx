...
```bash
# Stateless build
flowdeck build -w MyApp.xcworkspace -S "iPhone 16"
```
...
```bash
flowdeck build --workspace MyApp.xcworkspace --device "My Mac"
```
...
flowdeck build --workspace MyApp.xcworkspace --device "iPhone"
flowdeck build --workspace MyApp.xcworkspace --device "John's iPhone"

# By device UDID (exact match)
flowdeck build --workspace MyApp.xcworkspace --device "00008130-001245110C08001C"
```
...
```bash
# Debug build (default)
flowdeck build --workspace MyApp.xcworkspace --simulator "iPhone 16" --configuration Debug
...
If your project has multiple schemes:
...
```bash
flowdeck run --workspace MyApp.xcworkspace --device "My Mac"
```

### Build and Run on Physical Device

```bash
flowdeck run -w MyApp.xcworkspace -D "John's iPhone"
```
...
```bash
# Get build status
flowdeck build --workspace MyApp.xcworkspace --simulator "iPhone 16" --json | jq -s 'last'

# Check for errors
flowdeck build --workspace MyApp.xcworkspace --simulator "iPhone 16" --json | jq 'select(.level == "error")'
```
...
flowdeck build --workspace /path/to/MyApp.xcworkspace --simulator "iPhone 16"
