...
<Steps>
  ...
  <Step title="Xcode">
    ...
    - Download from the [Mac App Store](https://apps.apple.com/us/app/xcode/id497799835?mt=12) (15.0 or later)
...
```bash
curl -sSL https://flowdeck.studio/installer/cli.sh | sh
```

The installer will prompt you to choose an installation directory:

- **`/usr/local/bin`** (recommended) - Requires sudo, works immediately
- **`~/.local/bin`** - No sudo required, may need PATH configuration
...
FlowDeck CLI requires a valid license for full functionality.
...
```bash
# Remove the binary
sudo rm /usr/local/bin/flowdeck
