...
|--------|-------|-------------|
| `--platform <name>` | `-P` | Filter by platform (iOS, iPadOS, tvOS, watchOS, visionOS) |
...
Interactively select a simulator for the current project. Supports non-interactive selection for CI/CD.
...
# Select by exact UDID
flowdeck simulator select --udid 12345678-ABCD-EFGH-1234-567890ABCDEF
...
| `--platform <name>` | `-P` | Filter by platform |
| `--name <name>` | `-n` | Select simulator by name (non-interactive) |
| `--udid <udid>` | `-u` | Select simulator by UDID (non-interactive) |
